...
│   │   └── index.css             # index.astro用スタイル (867行)
│   │
│   ├── lib/
...
│   │
│   ├── pages/
│   │   ├── index.astro           # ホームページ (248行) ✅ CSS分離済
│   │   └── 404.astro             # 404ページ (7行)
│   │
...
| `pages/NewsList.tsx` | 94行→77行 | 低 | [x] ✅ 完了 |
| `pages/NewsDetail.tsx` | 40行→31行 | 低 | [x] ✅ 完了 |
| `pages/NoteDetail.tsx` | 141行→114行 | 低 | [x] ✅ 完了 |
| `Layout/Header.tsx` | 34行 | 低 | [x] ✅ 対象外（global.css使用） |
| `Layout/Footer.tsx` | 22行 | 低 | [x] ✅ 対象外（global.css使用） |
| `Layout/MobileNav.tsx` | 19行 | 低 | [x] ✅ 対象外（global.css使用） |
| `Countdown.tsx` | 146行→61行 | 低 | [x] ✅ 完了 |
| `App.tsx` | 83行 | 低 | [x] ✅ 対象外（スタイルなし） |

#### Astroファイル（CSS分離対象）
| ファイル | 行数 | 優先度 | ステータス |
|----------|------|--------|------------|
| `pages/index.astro` | 1,115行→248行 | 高 | [x] ✅ 完了 |
| `pages/404.astro` | 7行 | 低 | [x] ✅ 対象外（スタイルなし） |
...
#### Step 4: Reactコンポーネント（優先度低） ✅ 完了
- [x] `NewsList.tsx` → `NewsList.module.css` 作成・移行
- [x] `NewsDetail.tsx` → `NewsDetail.module.css` 作成・移行
- [x] `NoteDetail.tsx` → `NoteDetail.module.css` 作成・移行
- [x] `Countdown.tsx` → `Countdown.module.css` 作成・移行
- [x] Header/Footer/MobileNav/App → 対象外確認（global.css使用のため）

#### Step 5: Astroファイル ✅ 完了

- [x] `index.astro` の `<style>` を外部CSS化 → `src/styles/index.css` (867行)
